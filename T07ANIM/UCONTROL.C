...
static VOID MM3_UNITResponse( MM3UNIT_CONTROL *Uni, mm3ANIM *Ani )
{
  ...
  Uni->Pos = PointTransform(Uni->Pos, MatrRotateX(50 * Ani->JY * Ani->GlobalDeltaTime));
  Uni->Pos = PointTransform(Uni->Pos, MatrRotateY(50 * Ani->JX * Ani->GlobalDeltaTime));  

  if (Ani->Keys[VK_LBUTTON])
  {
    Uni->Pos = PointTransform(Uni->Pos, MatrRotateY(-5 * Ani->Mdx * Ani->GlobalDeltaTime));
    Uni->Pos = PointTransform(Uni->Pos, MatrRotateX(-5 * Ani->Mdy * Ani->GlobalDeltaTime));
  ...
  Uni->Pos = PointTransform(Uni->Pos, MatrRotateX(10 * Ani->Keys[VK_UP] * Ani->GlobalDeltaTime));
  Uni->Pos = PointTransform(Uni->Pos, MatrRotateX(-10 * Ani->Keys[VK_DOWN] * Ani->GlobalDeltaTime));
