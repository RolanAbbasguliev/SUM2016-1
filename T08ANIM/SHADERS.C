 ...
 * RETURNS: None.
 */
static VOID MM3_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("MM3{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
} /* End of 'MM3_RndShaderLoadTextFile' function */
 ...
 */
static CHAR * MM3_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
...
UINT MM3_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      MM3_RndShaderLog(suff[i], "Error with creating shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = MM3_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      MM3_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'MM3_RndShaderLoad' function */
...
VOID MM3_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'MM3_RndShaderFree' function */
